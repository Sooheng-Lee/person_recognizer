...
```python
{
    ...
    "center_x": 640,             # 중심점 X 좌표 (픽셀)
    "center_y": 360,             # 중심점 Y 좌표 (픽셀)
    "center_3d": {               # 3D 좌표 (미터 단위, 추정값)
        "x": 0.5,                # X: 수평 거리 (양수=오른쪽)
        "y": 0.2,                # Y: 수직 거리 (양수=아래)
        "z": 2.5                 # Z: 깊이 거리 (카메라로부터)
    },
    "depth_z": 2.5,              # 깊이 거리 (미터 단위, 추정값)
    "bbox": {
        "x1": 500, "y1": 200,    # 좌상단
        "x2": 780, "y2": 520,    # 우하단
        "width": 280,
...
단일 카메라에서 깊이를 추정하는 방법:

1. **핀홀 카메라 모델 사용**
   - 공식: `Z = (f × H_real) / h_pixel`
   - f: 초점 거리 (픽셀 단위, 카메라 FOV로 추정)
   - H_real: 평균 사람 키 (1.7m)
   - h_pixel: 바운딩 박스 높이 (픽셀)

2. **정확도 향상을 위한 캘리브레이션**
   ```python
   # 알려진 거리에서 캘리브레이션
   detector._depth_estimator.calibrate(
       known_distance=2.0,      # 사람까지 실제 거리 (미터)
       bbox_height=300,         # 그 때의 바운딩 박스 높이
       frame_height=1080        # 프레임 높이
   )
   ```
...
3. **제한 사항**
   - 추정값이므로 정확도는 ±30% 정도
   - 사람이 서 있을 때 가장 정확
   - 카메라 높이와 각도에 따라 오차 발생
   - 정확한 깊이가 필요하면 스테레오 카메라 또는 깊이 센서 필요
